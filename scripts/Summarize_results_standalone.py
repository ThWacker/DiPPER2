...
def run_tests(
    folder: Path, length: int, count: int, target_type: str, logger: Logger
) -> dict:
    """
    ...
    file_pattern = f"Primer_seqkit_amplicon_against_{target_type}_m*"
...
def handle_blasts_and_efetch(
    found_target: Path, logger: Logger
) -> str:
    """
    ...
    try:
        id, evalue, bitscore = get_highest_scoring_accession(found_target)
        evalue = format(evalue, ".2e")
        if id == "Blast did not find hits":
            return f"The primer's BLASTX search did not find any hits.\n"
    ...
    if is_tool("efetch"):
        ...
        return f"The primer's target with the highest bitscore {bitscore} & evalue {evalue} has the accession {id} and codes for {title.strip()}. \nFurther information on the accession: {accession} \n"
    else:
        accession = "https://www.ncbi.nlm.nih.gov/protein/" + id + "/"
        return f"The primer's target with the highest bitscore {bitscore} & evalue {evalue} has the accession {id}. \nFurther information on the accession: {accession} \n"
...
def generate_results(
    ...
    Args:
        forward (str): the forward primer
        reverse (str): the reverse primers
        target (Path): the file with the target
...
def main():
    ...
    check_folders(
        destination_folder_seqkit, logger=logger
    )
    ...
    # find if target file exists
    target = source_folder / "Primer_target.fasta"
    if not target.exists() and target.is_file():
        logger.exception("%s is not a file or does not exist", target)
        raise FileExistsError("%s does not exist or is not a file", target)
