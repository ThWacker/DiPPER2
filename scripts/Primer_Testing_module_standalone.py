...
def concat_files(folder: Path, name: str, source: Path, logger: Logger) -> Path:
    """
...
def main():
    now = datetime.now()
    dt_string = now.strftime("%d-%m-%Y_%z")

    parser = argparse.ArgumentParser(
        prog="DiPPER2",
        description="primer testing module. Primers generated by Primer3 are validated in silico. This is the standalone version.",
    ...
    parser.add_argument(
        ...
        default=dt_string,
        type=str,
        help="Outfile prefix. Default is date and time in d-m-y-tz format",
    )
    parser.add_argument(
        "-d",
        "--delete_concat",
    ...
    parser.add_argument(
        "-F", "--forward", required=True, type=str, help="Forward Primer"
    )
    parser.add_argument(
        "-R", "--reverse", required=True, type=str, help="Reverse Primer"
    )
    ...
    # get the primers
    pr_frwd = args.forward
    pr_rev = args.reverse

    # write primer to a file for convenience
    filename_p = source_folder / "Primer.fasta"
    try:
        out_prim = f">forward_primer\n{pr_frwd}\n>reverse_primer\n{pr_rev}\n"

        # Write output to the file
        with open(filename_p, "w", encoding="utf-8") as file:
            file.write(out_prim)
    except (IOError, OSError, PermissionError) as e:
        # Log error and raise exception with additional context
        logger.error(
            f"Error writing output of seqkit amplicon to file {filename_p}: {e}"
        )
        raise RuntimeError(
            f"Error writing output of seqkit amplicon to file {filename_p}: {e}"
        )
    ...
    for i in range(5):
        ...
        try:
            # Construct filename for output
            filename_n = (
                source_folder / f"Primer_seqkit_amplicon_against_neighbour_m{i}.txt"
            )
            ...
            # Log error and raise exception with additional context
            logger.error(
                f"Error writing output of seqkit amplicon to file {filename_n}: {e}"
            )
            raise RuntimeError(
                f"Error writing output of seqkit amplicon to file {filename_n}: {e}"
            )
    ...
    filename_t = source_folder / f"Primer_seqkit_amplicon_against_target_m0.txt"
    amp = get_amplicon(filename_t)  # Get the amplicon from the file
    logger.info(f"The amplicon is {amp}")

    # write amplicon to file
    try:
        # Construct filename for output
        filename_a = source_folder / "Primer_target.fasta"
        out_amp = f">target\n{amp}"

        # Write output to the file
        with open(filename_a, "w", encoding="utf-8") as file:
            file.write(out_amp)
    except (IOError, OSError, PermissionError) as e:
        # Log error and raise exception with additional context
        logger.error(
            f"Error writing output of seqkit amplicon to file {filename_t}: {e}"
        )
        raise RuntimeError(
            f"Error writing output of seqkit amplicon to file {filename_t}: {e}"
        )
    ...
    try:
        ...
        result = subprocess.run(
            [
                ...
                "-query",
                str(filename_a),  # Input file (query)
                "-remote",  # Use remote database (instead of local)
                "-db",
                "nr",  # Database to query against (nr - non-redundant)
                "-evalue",
                "0.00001",  # E-value threshold for the alignment
                "-outfmt",
                "6",  # Output format (tabular)
            ],
            stdout=subprocess.PIPE,  # Capture standard output
            stderr=subprocess.PIPE,  # Capture standard error
    ...
    if not output_tar:
        ...
        try:
            if args.ref:
                ...
                seqk_loc_out = run_seqkit_locate(
                    amp, ref, logger
                )  # Run seqkit locate with the reference
            else:
                logger.warning("No reference found, using longest target assembly")
                try:
                    ref = get_longest_target(
                        fur_target
                    )  # Get the longest target assembly

                except Exception as e:
                    logger.warning(f"Could not find a reference: {e}")
                    raise Exception(f"Could not find a reference: {e}")
                logger.info(f"Using longest target assembly: {ref}")
                seqk_loc_out = run_seqkit_locate(
                    amp, ref, logger
                )  # Run seqkit locate with the longest assembly
    ...
    try:
        ...
        filenamed = f"{filename_a}_blastx_1e-5.txt"
    ...
    in_silico_folder = source_folder / "in_silico_tests"
    in_silico_folder.mkdir(
        parents=True, exist_ok=True
    )  # Create the subfolder if it doesn't exist
    pattern_to_match = (
        "seqkit_amplicon_against"  # Pattern to search for in the file names
    )

    logger.info(
        f"Moving files with {pattern_to_match} in name from {source_folder} into {in_silico_folder}..."
    )

    try:
        # Move files matching the pattern from the destination folder to the in_silico_tests folder
        move_files_with_pattern(source_folder, pattern_to_match, in_silico_folder)
    except Exception as e:
        # If an error occurs during the file moving process, log it and raise an exception
        logger.error(
            f"Error moving files with {pattern_to_match} in name from {source_folder} into {in_silico_folder}: {e}"
        )
        raise Exception(
            f"Error moving files with {pattern_to_match} in name from {source_folder} into {in_silico_folder}: {e}"
        ) from e
