...
def main():
   ...
   # Log an informational message to indicate that the blastx command is starting
    ...
    amp = get_amplicon(filename)  # Get the amplicon from the file
    ...
    try:
        ...
        result = subprocess.run(
            [
                ...
                "-query", str(filename_a),  # Input file (query)
    ...
    logger.info(f"Moving files with {pattern_to_match} in name from {source_folder} into {in_silico_folder}...")
    ...
    try:
        ...
        logger.error(f"Error moving files with {pattern_to_match} in name from {source_folder} into {in_silico_folder}: {e}")
        raise Exception(f"Error moving files with {pattern_to_match} in name from {source_folder} into {in_silico_folder}: {e}") from e
