...
diemsg() {
    echo "USAGE:
    $0 -f <results folder> -d <folder with the assemblies> -l <list with targets> 
    OPTIONAL:
...
if [[ -n $OUT && -n $REF && -n $M_MAX && -n $M_MIN && -n $INT && -n $WORK ]]; then
    "$default_python"  "$SCRIPT_DIR"/Primer_Testing_module_optimized.py -f "$FOLD" -o "$OUT" -r "$REF" -m "$M_MAX" -d "$M_MIN" -c "$INT" -w "$WORK" && true
    EXIT_STATUS="$?"
elif [[ -n $OUT && -n $REF && -n $M_MAX && -n $M_MIN && -n $INT ]]; then
    "$default_python"  "$SCRIPT_DIR"/Primer_Testing_module_optimized.py -f "$FOLD" -o "$OUT" -r "$REF" -m "$M_MAX" -d "$M_MIN" -c "$INT" && true
    EXIT_STATUS="$?"
elif [[ -n $OUT && -n $REF && -n $M_MAX && -n $M_MIN ]]; then
    "$default_python"  "$SCRIPT_DIR"/Primer_Testing_module_optimized.py -f "$FOLD" -o "$OUT" -r "$REF" -m "$M_MAX" -d "$M_MIN"  && true
    EXIT_STATUS="$?"  
elif [[ -n $OUT && -n $REF && -n $M_MAX && -n $M_MIN ]]; then
    "$default_python"  "$SCRIPT_DIR"/Primer_Testing_module_optimized.py -f "$FOLD" -o "$OUT" -r "$REF" -m "$M_MAX" -d "$M_MIN"  && true
    EXIT_STATUS="$?"
elif [[ -n $OUT && -n $REF && -n $M_MAX ]]; then
    "$default_python"  "$SCRIPT_DIR"/Primer_Testing_module_optimized.py -f "$FOLD" -o "$OUT" -r "$REF" -m "$M_MAX"  && true
    EXIT_STATUS="$?"
