...
from Primer3_module_optimized import (move_files,parse_primers,find_and_return_following_lines_and_target, main)
...
class TestMoveFolders(unittest.TestCase):
    ...
    def testMoveFoldersSuccess(self, mock_glob, mock_move, mock_logger_class):
        ...
        move_files(Path(self.source_dir), Path(self.destination_dir), "*.fasta", mock_logger_instance)
        
        # Check if shutil.move is called correctly
        self.assertEqual(mock_move.call_count, 3)
        mock_move.assert_any_call(Path(self.file1), Path(self.destination_dir, "file1.fasta"))
        mock_move.assert_any_call(Path(self.file2), Path(self.destination_dir, "file2.fasta"))
        mock_move.assert_any_call(Path(self.file3), Path(self.destination_dir, "file3.fasta"))
    ...
    def testMoveFoldersLogger(self, mock_logger_class, mock_glob, mock_move):
        ...
        self.assertTrue(f"Moved {Path(self.file1)} to {Path(self.destination_dir)}" in mock_logger_instance.info.call_args[0][0])
    ...
    def testMoveFolderFailureNoGlob(self, mock_logger_class, mock_glob, mock_move):
        ...
        with self.assertRaises(FileNotFoundError):
            ...
            # Call the function being tested
            move_files(Path(self.source_dir), Path(self.destination_dir), "*.fasta", logger)

        # Check if logger.exception was called with the info message
        mock_logger_instance.exception.assert_called_once()
        self.assertTrue(
            "No files matching *.fasta found in" in mock_logger_instance.exception.call_args[0][0]
        )

    @patch("Primer3_module_optimized.shutil.move")
    @patch("Primer3_module_optimized.Path.glob")
    @patch('target_move_module_optimized.Logger')
    def testMoveFolderFailureFileNotFound(self, mock_logger_class, mock_glob, mock_move):
        # Create a mock logger instance
        mock_logger_instance = MagicMock()
        mock_logger_class.return_value.get_logger.return_value = mock_logger_instance
        ...
        with patch.object(Path, "glob", return_value=[Path("/mock/DieQuelle/file1.fasta")]):
            # Assert that FileNotFoundError is raised
            with self.assertRaises(FileNotFoundError):
        ...
        mock_move.assert_called_once_with(
            Path("/mock/DieQuelle/file1.fasta"), Path("/mock/FinalDestination/file1.fasta")
        )
...
class testParsePrimers(unittest.TestCase):
    ...
    def testParsePrimersSuccess(self,mock_logger_class):
        ...
        #expected result
        expect=[['PRIMER_PAIR_0_PENALTY', '0.12345'],['PRIMER_PAIR_0_PENALTY', '0.456789'],['PRIMER_PAIR_0_PENALTY', '0.56789'],['PRIMER_PAIR_0_PENALTY', '0.891011']]
